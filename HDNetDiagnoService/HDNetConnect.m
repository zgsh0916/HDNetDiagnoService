...
- (void)runWithHostAddress:(NSString *)hostAddress port:(int)port
{
    ...
    _startTime = [HDNetTimer getMicroSeconds];
    [self connect];
    do {
                                     ...
                                     CFDataRef address, const void *data, void *info)
{
    if (data != NULL) {
        printf("connect");
        HDNetConnect *con = (__bridge_transfer HDNetConnect *)info;
...
- (void)readStream:(BOOL)success
{
    ...
    if (success) {
        ...
        NSInteger interval = [HDNetTimer computeDurationSince:_startTime] / 1000;
        _sumTime += interval;
