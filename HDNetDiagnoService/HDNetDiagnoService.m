//
//  HDNetDiagnoService.m
//  HDNetDiagnoServieDemo
//
...
#import "HDNetDiagnoService.h"
#import "HDNetPing.h"
#import "HDNetTraceRoute.h"
#import "HDNetGetAddress.h"
#import "HDNetTimer.h"
#import "HDNetConnect.h"

static NSString *const kPingOpenServerIP = @"test.venus.wxhundun.com";
static NSString *const kCheckOutIPURL = @"";

@interface HDNetDiagnoService () <HDNetPingDelegate, HDNetTraceRouteDelegate,
                                  HDNetConnectDelegate> {
    ...
    HDNetPing *_netPinger;
    HDNetTraceRoute *_traceRouter;
    HDNetConnect *_netConnect;
}

@end

@implementation HDNetDiagnoService
        ...
        MobileNetCode:(NSString *)theMobileNetCode
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateNetStatus:) name:@"NetChange" object:nil];
    }

    return self;
}

- (void)updateNetStatus:(NSNotification *)noti {
    NSString *netType = noti.userInfo[@"netType"];
    
    if ([netType isEqualToString:@"unkown"] || [netType isEqualToString:@"unreachable"]) {
        _curNetType = NETWORK_TYPE_NONE;
    }else if ([netType isEqualToString:@"wwan"]) {
         
        _curNetType = [self getNetType];
    }else if ([netType isEqualToString:@"wifi"]) {
...
//        [self recordOutIPInfo];
    ...
    if (_isRunning) {
        ...
        if ([_hostAddress count] > 0) {
            _netConnect = [[HDNetConnect alloc] init];
    ...
    if (_isRunning) {
        //开始诊断traceRoute
        [self recordStepInfo:@"\n开始traceroute..."];
        _traceRouter = [[HDNetTraceRoute alloc] initWithMaxTTL:TRACEROUTE_MAX_TTL
...
- (void)recordLocalNetEnvironment
{
    ...
    _dnsServers = [NSArray arrayWithArray:[HDNetGetAddress outPutDNSServers]];
    ...
    // host地址IP列表
    long time_start = [HDNetTimer getMicroSeconds];
    _hostAddress = [NSArray arrayWithArray:[HDNetGetAddress getDNSsWithDormain:_dormain]];
    long time_duration = [HDNetTimer computeDurationSince:time_start] / 1000;
    if ([_hostAddress count] == 0) {
...
- (void)recordOutIPInfo
{
    ...
    NSURLSessionDataTask *task = [[NSURLSession sharedSession] dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        NSString *responString = [[NSString alloc] initWithData:data encoding:0x80000632];
        [self recordStepInfo:responString];
    }];
    [task resume];
...
- (void)pingDialogsis:(BOOL)pingLocal
{
    ...
    [self recordStepInfo:@"\n开始ping..."];
    _netPinger = [[HDNetPing alloc] init];
