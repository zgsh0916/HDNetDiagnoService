...
@implementation HDNetTraceRoute
...
- (Boolean)doTraceRoute:(NSString *)host
{
    ...
    NSArray *serverDNSs = [HDNetGetAddress getDNSsWithDormain:host];
    if (!serverDNSs || serverDNSs.count <= 0) {
        if (_delegate != nil) {
            [_delegate appendRouteLog:@"TraceRoute>>> CouHD not get host address"];
    ...
    if ((recv_sock = socket(destination->sa_family, SOCK_DGRAM, isIPV6?IPPROTO_ICMPV6:IPPROTO_ICMP)) < 0) {
        if (_delegate != nil) {
            [_delegate appendRouteLog:@"TraceRoute>>> CouHD not create recv socket"];
            [_delegate traceRouteDidEnd];
        }
        return false;
    }
    ...
    if ((send_sock = socket(destination->sa_family, SOCK_DGRAM, 0)) < 0) {
        if (_delegate != nil) {
            [_delegate appendRouteLog:@"TraceRoute>>> CouHD not create xmit socket"];
            [_delegate traceRouteDidEnd];
        }
        return false;
    }
    ...
    while (ttl <= maxTTL) {
        ...
        for (int try = 0; try < maxAttempts; try ++) {
            startTime = [HDNetTimer getMicroSeconds];
            //发送成功返回值等于发送消息的长度
            ...
            if (FD_ISSET(recv_sock, &readfds) > 0) {
                ...
                if ((res = recvfrom(recv_sock, buf, 100, 0, (struct sockaddr *)&fromAddr, &n)) <
                    ...
                    icmp = true;
                    delta = [HDNetTimer computeDurationSince:startTime];
